import type { Metadata } from "next";
import { whatsappLink } from "@/lib/utils";
import { BANK_INFO } from "@/lib/bankInfo";

export const metadata: Metadata = {
  title: "Medios de pago",
  description: "Conocé los medios de pago disponibles en Herrajes Darío: transferencia bancaria y efectivo.",
};

export default function MediosDePagoPage() {
  return (
    <div className="container-hd py-10">
      <h1 className="section-title">Medios de pago</h1>
      <p className="mt-2 max-w-2xl text-sm text-gray-600">
        Elegí la forma de pago que más te convenga. Coordinamos cada compra
        por WhatsApp.
      </p>

      <div className="mt-10 grid gap-6 sm:grid-cols-2">
        <div className="rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-bold text-black">Transferencia bancaria</h2>
          <p className="mt-2 text-sm text-gray-600">Cuenta Corriente</p>
          <dl className="mt-3 space-y-1 text-sm">
            <div className="flex justify-between gap-3">
              <dt className="text-gray-500">Titular</dt>
              <dd className="font-semibold text-black">{BANK_INFO.titular}</dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-gray-500">CUIL</dt>
              <dd className="font-semibold text-black">{BANK_INFO.cuil}</dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-gray-500">CBU</dt>
              <dd className="font-semibold text-black">{BANK_INFO.cbu}</dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-gray-500">Alias</dt>
              <dd className="font-semibold text-black">{BANK_INFO.alias}</dd>
            </div>
          </dl>
          <p className="mt-3 text-xs text-gray-500">
            Enviá el comprobante por WhatsApp para confirmar tu pedido.
          </p>
        </div>
        <div className="rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-bold text-black">Efectivo</h2>
          <p className="mt-2 text-sm text-gray-600">
            Pagá en efectivo al retirar tu pedido en nuestro local de Burzaco.
          </p>
        </div>
      </div>

      <div className="mt-10 rounded-lg border border-gray-200 bg-gray-50 p-6">
        <h2 className="text-lg font-bold text-black">¿Tenés dudas sobre tu pago?</h2>
        <p className="mt-2 text-sm text-gray-600">
          Escribinos por WhatsApp y te ayudamos a confirmar tu pedido.
        </p>
        <a
          href={whatsappLink("Hola! Tengo una consulta sobre medios de pago.")}
          target="_blank"
          rel="noopener noreferrer"
          className="btn-primary mt-4 inline-flex"
        >
          Consultar por WhatsApp
        </a>
      </div>
    </div>
  );
}
